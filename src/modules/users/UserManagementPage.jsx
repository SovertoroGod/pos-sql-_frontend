...
import { Search, Users, Filter, ChevronLeft, ChevronRight, UserPlus, Edit, Trash2, MoreVertical, Eye } from 'lucide-react';
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-2">Manage and organize your system users</p>
          </div>
              ...
              {users.length === 0 ? (
                ...
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">{user.branch_id || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button className="p-1 text-gray-400 hover:text-red-600 transition-colors duration-150" title="Delete User">
                          <Trash2 className="h-4 w-4" />
                        </button>
                        <button className="p-1 text-gray-400 hover:text-gray-600 transition-colors duration-150" title="More Options">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-sm text-gray-700">
                {metadata.totalItems > 0 ? (
                  <>
                    Showing {((filters.page - 1) * (metadata.limit || 10)) + 1} to{' '}
                    {Math.min(filters.page * (metadata.limit || 10), metadata.totalItems || 0)} of{' '}
                    {metadata.totalItems || 0} results
                  </>
                ) : (
                  <>No results found</>
                )}
              ...
              <div className="flex items-center gap-2">
                ...
                {Array.from({ length: metadata.totalPages || 1 }, (_, i) => {
                  ...
                  const isNearCurrent = Math.abs(page - filters.page) <= 2 || page === 1 || page === metadata.totalPages;
                  
                  if (!isNearCurrent && i > 0 && i < metadata.totalPages - 1) return null;
